...
const sortPopular = (list, ids, key = "id") =>
  [...list].sort((a, b) => {
    const av = (a[key] || a.id)?.toLowerCase();
    const bv = (b[key] || b.id)?.toLowerCase();
    const ai = ids.indexOf(av);
    const bi = ids.indexOf(bv);
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        // Fetch from each active provider in parallel; each result is tagged
        // with that provider so the user can see who is offering what price.
        const providerList =
          activeProviders.length > 0 ? activeProviders : [null];
        ...
        results.forEach((result, providerIndex) => {
          ...
          Object.entries(data)
            ...
            .forEach(([key, val]) => {
              ...
              list.push({
                // Unique key: serviceId + providerId so WhatsApp from 2 providers = 2 cards
        ...
        setServices(sortPopular(list, POPULAR_SERVICE_IDS, "serviceId"));
      } catch {
      ...
      try {
        ...
        if (ordersRes.status === "fulfilled") {
          ...
          setTotalOtps(
            Array.isArray(orders)
              ? orders.filter(
                  (o) => o.status === "RECEIVED" || o.status === "FINISHED",
                ).length
              : 0,
          );
        }
        if (txRes.status === "fulfilled") {
          const txData =
            txRes.value.data?.transactions || txRes.value.data || [];
  ...
  const submitOrder = async () => {
    ...
    try {
      const body = {
        ...
        product: selectedService.serviceId, // base service ID, not the composite key
        operator: "any",
      };
      // Pass the preferred provider so the backend targets it directly
      if (selectedService.providerId) {
        body.provider = selectedService.providerId;
      }

      const { data } = await api.post("/api/numbers/buy", body, {
  ...
  const popularCountries = filteredCountries.filter((c) =>
    POPULAR_COUNTRY_IDS.includes(c.id),
  );
  const otherCountries = filteredCountries.filter(
    (c) => !POPULAR_COUNTRY_IDS.includes(c.id),
  );

  // Popular / other split uses serviceId (the base key) for matching
  const popularServices = services.filter((s) =>
    POPULAR_SERVICE_IDS.includes((s.serviceId || s.id).toLowerCase()),
  );
  const otherServices = services.filter(
    (s) => !POPULAR_SERVICE_IDS.includes((s.serviceId || s.id).toLowerCase()),
  );
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Get an instant number for verification.
          </p>
        </div>
      </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Grow your followers, likes &amp; views on Instagram, TikTok,
                YouTube and more.
              </p>
            </div>
          </div>
            ...
            <CardContent className="p-0">
              ...
              <div className="flex border-b border-border">
                {[1, 2, 3].map((s) => (
                  <div
                    key={s}
                    className={`flex-1 py-4 text-center text-sm font-medium transition-colors ${step >= s ? "text-primary" : "text-muted-foreground"}`}
                  >
                    {s === 1
                      ? "1. Country"
                      : s === 2
                    ...
                    {!countriesLoading && !countriesError && (
                      <div className="mb-5">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {countries
                            .filter((c) => POPULAR_COUNTRY_IDS.includes(c.id))
                            .map((c) => (
                              <button
                                key={c.id}
                                onClick={() => {
                          ...
                          >
                            {country.flag ? (
                              <img
                                src={country.flag}
                                alt={country.name}
                                className="w-6 h-auto mr-3 rounded-sm shadow-sm"
                                onError={(e) => {
                                  e.target.style.display = "none";
                                }}
                              />
                    ...
                    {activeProviders.length > 1 &&
                      ...
                      services.length > 0 && (
                        <p className="text-xs text-muted-foreground mb-3 px-1">
                          Showing prices from {activeProviders.length} providers
                ...
                {step === 3 && (
                  <div className="animate-in fade-in slide-in-from-right-4 duration-300">
                    ...
                    <div className="bg-card/50 border border-border rounded-xl p-5 mb-6">
                      <h3 className="font-semibold mb-4 border-b border-border pb-3">
                        Order Summary
                      </h3>
                    ...
                    <div className="text-center mb-6">
                      <p className="text-sm text-muted-foreground mb-4">
                        Enter your 4-digit PIN to confirm purchase
                      </p>
                ...
                {step === 4 && (
                  <div className="py-12 flex flex-col items-center justify-center animate-in fade-in duration-300">
                    ...
                    <p className="text-muted-foreground text-center max-w-sm">
                      Assigning your number. This only takes a moment.
                    </p>
                  </div>
                )}
                ...
                transactions.map((tx, i) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div
                        className={`w-10 h-10 rounded-full flex items-center justify-center ${tx.type === "Credit" || tx.type === "credit" ? "bg-emerald-500/10 text-emerald-500" : "bg-primary/10 text-primary"}`}
                      >
                    ...
                    <div
                      className={`font-bold ${tx.type === "Credit" || tx.type === "credit" ? "text-emerald-500" : ""}`}
                    >
                      {tx.type === "Credit" || tx.type === "credit" ? "+" : "-"}
                      ₦{tx.amount}
                    </div>
