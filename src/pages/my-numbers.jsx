...
function getOtp(order) {
  if (Array.isArray(order.sms) && order.sms.length > 0) {
    ...
    return {
      ...
      text: latest.text || latest.smstext || null,
    };
  }
  return null;
}
...
function getEffectiveStatus(entry) {
  ...
  if (entry.otp?.code) return "received";
  if (
    entry.expiresAt &&
...
function TimerBar({ expiresAt, purchasedAt, status }) {
  ...
  const total   = 15 * 60; // matches ORDER_EXPIRY_MS on the backend
  ...
  const barColor  = urgent ? "bg-red-500"    : warning ? "bg-amber-500"    : "bg-emerald-500";
  const textColor = urgent ? "text-red-500"  : warning ? "text-amber-500"  : "text-emerald-500";
...
function SmsSection({ entry, onCheck, checking }) {
  const hasOtp    = !!entry.otp?.code;
  const msgCount  = entry.smsAll?.length || (entry.otp ? 1 : 0);
...
export default function MyNumbers() {
  ...
  // Tracks which order IDs are being auto-polled
  // so each card's refresh icon spins independently
  const [pollingIds, setPollingIds] = useState(new Set());
  const isPolling = pollingIds.size > 0;

  // Stable ref — polling interval reads latest numbers
  // without needing to restart when numbers changes
  ...
  const load = async () => {
    ...
    try {
      ...
      const mapped = orders.map((order) => {
        const otp    = getOtp(order);
        const status = normalizeStatus(order.status);

        // Fallback: if backend doesn't return expiresAt, derive it
        // from createdAt + 15 min so the timer always has a value
        const fallbackExpiry = order.createdAt
          ? new Date(new Date(order.createdAt).getTime() + 15 * 60 * 1000).toISOString()
          : null;
        ...
        const mappedOrder = {
          ...
          expiresAt:   order.expiresAt || fallbackExpiry,
        };

        mappedOrder.status = getEffectiveStatus(mappedOrder);
        return mappedOrder;
      });
  ...
  const checkSms = async (entry, showError = true) => {
    try {
      const { data } = await api.get(`/api/numbers/check/${entry.orderId}`);
      if (!data) return null;
      ...
      setNumbers((prev) =>
        prev.map((n) => {
          ...
          const updated = {
            ...
            otp:      getOtp(data),
            smsAll:   Array.isArray(data.sms) ? data.sms : n.smsAll,
            // Pull expiresAt from poll response so the timer always
            // has the correct value even on the very first check
            expiresAt: data.expiresAt || n.expiresAt,
          };

          updated.status = getEffectiveStatus(updated);
          return updated;
        })
      );
      ...
      console.error(`Failed to check SMS for order ${entry.orderId}:`, err);
      if (showError) {
        setError(err?.response?.data?.message || "Failed to check for SMS.");
      }
      return null;
    }
  };
  ...
  // ── MANUAL SMS CHECK ───────────────────────────────────────
  const handleManualCheckSms = async (entry) => {
    setCheckingId(entry.id);
    try {
      await checkSms(entry, true);
    } finally {
      setCheckingId(null);
    }
  };
  ...
  // ── AUTO POLLING ───────────────────────────────────────────
  // Runs once on mount. Uses numbersRef so it always has the
  // latest list without restarting the interval on every update.
  useEffect(() => {
    const pollSms = async () => {
      // Poll waiting AND received orders until a code arrives
      const activeOrders = numbersRef.current.filter(
        (o) => isActive(o.status) && !o.otp?.code
      );

      if (activeOrders.length === 0) return;

      // Mark each order's card icon as spinning
      setPollingIds(new Set(activeOrders.map((o) => o.id)));

      for (const order of activeOrders) {
        await checkSms(order, false);
      }
    ...
    return () => {
      ...
      setPollingIds(new Set()); // ← was the bug: "setIsPolling(false)" — that function doesn't exist
    };
    ...
    const intervalId = setInterval(() => {
      setNumbers((prev) =>
        prev.map((entry) => {
          ...
          return { ...entry, status: getEffectiveStatus(entry) };
        })
      );
    }, 1000);
          ...
          <div className="flex items-center justify-between flex-wrap gap-4">
            ...
            <div className="flex gap-2">
              ...
              >
                ...
                {loading ? "Refreshing…" : isPolling ? "Checking SMS…" : "Refresh"}
              </button>
        ...
        <AnimatePresence>
          <div className="space-y-4">
            {filtered.map((entry, i) => {
              ...
              const isThisPolling = isChecking || pollingIds.has(entry.id);
                ...
                >
                  ...
                  <div className="flex items-start justify-between gap-3 flex-wrap">
                    ...
                    <div className="flex items-center gap-1.5 flex-shrink-0">
                      ...
                      {isActive(entry.status) && (
                        <button
                          onClick={() => handleManualCheckSms(entry)}
                          disabled={isChecking}
                        ...
                        >
                          <RefreshCw className={`w-4 h-4 ${isThisPolling ? "animate-spin" : ""}`} />
                        </button>
                      )}
                    </div>
                    ...
                    {entry.smsAll?.length > 0 && (
                      <button
                        onClick={() => {
                          ...
                          a.href     = url;
                          a.download = `sms-${entry.number}.txt`;
                          a.click();
